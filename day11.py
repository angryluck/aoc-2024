...
def transform(num: int) -> [int]:
    ...
    if digits%2 == 0:
        return [int(digit_string[:digits//2]), int(digit_string[digits//2:])]
    return [int(num)*2024]

def blink(xs:[str]) -> [str]:
    ret = []
    for num in xs:
...
def stone_count_automatic(n:int, x:int) -> int:
    ...
    if digits%2 == 0:
        ...
        right = x % (10**tmp)
        left = x // (10**tmp)
        return stone_count_manual(n-1, right) + stone_count_manual(n-1, left)
