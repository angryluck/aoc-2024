...
def parse_rule(rules: Rules, text: str) -> None:
...
def parse(text: str) -> tuple[Rules, list[Update]]:
...
# Part 1
def is_in_order(rules: Rules, updates: Update) -> bool:
    return all(
        all(lt(rules, p1, p2) for p2 in updates[i + 1 :])
        for i, p1 in enumerate(updates)
    )


def middle_page_number(xs: list[int]) -> int:
    return xs[len(xs) // 2]
...
def move_forward(i: int, xs: list) -> None:
    tmp1 = xs[i]
    tmp2 = xs[i - 1]
    xs[i] = tmp2
    xs[i - 1] = tmp1
...
def sort(rules: Rules, xs: list[int]) -> list[int]:
    ret = list(xs)
...
def part2(text: str) -> int:
    ...
    return sum(
        middle_page_number(sort(rules, line)) for line in updates_not_in_order
    )
